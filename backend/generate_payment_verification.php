...
$invoiceDate = trim($data['invoice_date'] ?? $data['payment_verification_date'] ?? date('Y-m-d'));
...
try {
    ...
    $conn->exec("
        CREATE TABLE IF NOT EXISTS payment_verifications (
            ...
            payment_verification_year INT NOT NULL,
    ...
    $invoiceColumns = $conn->query("SHOW COLUMNS FROM payment_verifications")->fetchAll(PDO::FETCH_COLUMN);
    if (!in_array('manual_student_name', $invoiceColumns, true)) {
        $conn->exec("ALTER TABLE payment_verifications ADD COLUMN manual_student_name VARCHAR(255) DEFAULT NULL AFTER student_id");
    }
    $studentIdColumn = $conn->query("SHOW COLUMNS FROM payment_verifications LIKE 'student_id'")->fetch(PDO::FETCH_ASSOC);
    ...
    if ($lastNumber === false) {
        $lastNumber = 0;
        $insertSeq = $conn->prepare("INSERT INTO payment_verification_sequence (payment_verification_year, last_number) VALUES (?, 0)");
    ...
    $updateSeq = $conn->prepare("UPDATE payment_verification_sequence SET last_number = ? WHERE payment_verification_year = ?");
    $updateSeq->execute([$sequence, $year]);

    $invoiceDir = __DIR__ . '/payment_verifications/';
    if (!is_dir($invoiceDir)) {
        mkdir($invoiceDir, 0755, true);
    }

    $safeInvoice = str_replace('/', '-', $invoiceNumber);
    $fileName = "payment_verification_{$safeInvoice}.pdf";
    ...
    $insertInvoice = $conn->prepare("
        ...
        (payment_verification_number, payment_verification_year, sequence_number, student_id, manual_student_name, course_id, description, unit_price, vat_amount, subtotal, total, payment_verification_date, file_path)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    ...
    record_audit_log(
        ...
        "payment_verification_generated",
        "payment_verification",
        null,
        $invoiceNumber,
        "Generated payment verification for {$student['student_name']}",
        [
            "payment_verification_number" => $invoiceNumber,
    ...
    echo json_encode([
        ...
        "document_url" => "payment_verifications/" . basename($filePath),
        "payment_verification_number" => $invoiceNumber,
        "payment_verification_url" => "payment_verifications/" . basename($filePath)
    ]);
} catch (Exception $e) {
